...
public class DateTimeUtil {
    ...
    public static LocalTime parserStringComSegundosParaLocalTime(String padraoHora) {
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern(FORMATO_HORA_MINUTO_SEGUNDO_BRASIL);
        return LocalTime.parse(padraoHora, formatter);
    }

    public static LocalDate parserStringBrasilParaLocalDate(String padraoHora) {
    ...
    public static Long converterLocalDateParaEpochTime(LocalDateTime time) {
        if (time != null) {
    ...
    public static Integer getIdade(LocalDate dataNascimento) {
        if (Objects.nonNull(dataNascimento)) {
            return Period.between(dataNascimento, DateTimeUtil.obterDataAtualDeBrasilia()).getYears();
        } else {
            return 0;
        }
    }
